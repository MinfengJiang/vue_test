    ...
    <el-row>
      <div class="infoBox">
        <div class="infoBoxHeader">
          <span>{{ $t('auditLog.auditLogTitle') }}</span>
          <el-button type="primary" plain size="mini">{{ $t('auditLog.btn1') }}</el-button>
          <el-button type="primary" plain size="mini">{{ $t('auditLog.btn2') }}</el-button>
        </div>
        <el-table :data="list" style="width: 100%;padding-top: 10px;" :default-sort="{prop: 'date', order: 'descending'}">
          <el-table-column :label="$t('auditLog.Num')" width="60" prop="id" sortable>
            <template slot-scope="scope">{{ scope.row.Num }}</template>
          </el-table-column>
          <el-table-column :label="$t('auditLog.Hostname')" width="130">
            <template slot-scope="scope">{{ scope.row.Hostname }}</template>
          </el-table-column>
          <el-table-column :label="$t('auditLog.UserName')" width="140">
            <template slot-scope="scope">{{ scope.row.UserName }}</template>
          </el-table-column>
          <el-table-column :label="$t('auditLog.ServiceName')" width="130">
            <template slot-scope="scope">{{ scope.row.ServiceName }}</template>
          </el-table-column>
          <el-table-column :label="$t('auditLog.Client_ip')" width="160">
            <template slot-scope="scope">{{ scope.row.Client_ip }}</template>
          </el-table-column>
          <el-table-column :label="$t('auditLog.Operation')" width="130">
            <template slot-scope="scope">{{ scope.row.Operation }}</template>
          </el-table-column>
          <el-table-column :label="$t('auditLog.Time')">
